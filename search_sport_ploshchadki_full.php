...
<div class = "div_history_bd" style= "position: absolute; left: 8%;top:30%; width: 80%; border:none;">
    <table style= "position: absolute; left: 0%;top:50%;">
        ...
        if ($_POST['adress'] <> NULL){
            $adress = $_POST['adress'];
            $products = mysqli_query($mysql, "SELECT * FROM `sport_ploshchadki_full` WHERE adress LIKE '%$adress%'");
        ...
        if ($_POST['objectname'] <> NULL){
            ...
            $products = mysqli_query($mysql, "SELECT * FROM `sport_ploshchadki_full` WHERE objectname LIKE '%$objectname%'");
            $products = mysqli_fetch_all($products);
            foreach ($products as $product) {
                echo'
                    <tr>
        ...
        if ($_POST['Dicttrict'] <> NULL){
            ...
            $products = mysqli_query($mysql, "SELECT * FROM `sport_ploshchadki_full` WHERE Dicttrict LIKE '%$Dicttrict%'");
            $products = mysqli_fetch_all($products);
            foreach ($products as $product) {
                echo'
                    <tr>
        ...
        if ($_POST['AdmArea'] <> NULL){
            ...
            $products = mysqli_query($mysql, "SELECT * FROM `sport_ploshchadki_full` WHERE AdmArea LIKE '%$AdmArea%'");
            $products = mysqli_fetch_all($products);
            foreach ($products as $product) {
                echo'
                    <tr>
