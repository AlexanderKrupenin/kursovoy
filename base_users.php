...
<div class = "div_history_bd" style= "position: absolute; left: 8%;top:30%; width: 80%; border:none;">
        ...
        <div>
        <input style='background: linear-gradient(132deg, #90dbf4, #cfbaf0, #fde4cf);' type=\"text\" name=\"id_user\" placeholder='ID'>
        </div><br>
        ...
        <div class = 'div_history_bd' style= 'position: absolute; left: 10%;top:195%; width:10%; border:none;'>
        <div>
        ...
        <div>
        <input style='background: linear-gradient(132deg, #90dbf4, #cfbaf0, #fde4cf);' type=\"text\" name=\"role_up\" placeholder='Роль'>
        </div>
        
        <div><br>
        <input style='background: linear-gradient(132deg, #90dbf4, #cfbaf0, #fde4cf);' type=\"text\" name=\"ver_up\" placeholder='Верефикатор'>
        </div><br>
        ...
        <div class = 'div_history_bd' style= 'position: absolute; left: 8%;top:195%; width:10%; border:none;'>
        <div>
        ...
        $id_user_del = $_POST['id_user_del'];
            ...
            && $ver_up!="" && $login_up!=""){
            $update_users = mysqli_query($mysql, "UPDATE `users` SET `id_user` = '$id_user_up',
             `first_name` = '$first_name_up', `second_name` = '$second_name_up', 
            `surname` = '$surname_up', `role` = '$role_up', `ver` = '$ver_up', `login` = '$login_up' WHERE `users`.`id_user` = '$id_user_up'");
            }

            $products = mysqli_query($mysql, "SELECT * FROM `users` WHERE id_user = '$id_user'  
        ...
        if ($products != "") {
            $products = mysqli_query($mysql, "SELECT * FROM `users` WHERE id_user LIKE '%$id_user%'  
            AND first_name LIKE '%$first_name%' AND second_name LIKE '%$second_name%' AND surname LIKE '%$surname%' AND role LIKE '%$role%'
            AND ver LIKE '%$ver%' AND login LIKE '%$login%'");
